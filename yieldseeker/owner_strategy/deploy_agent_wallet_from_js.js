...
const AGENT_STRATEGY = "0xD2792E8F0dDABeB263B7F269aaaac02B1D1fFDD9"; // AGENT STRATEGY
...
const username = `user_${AGENT_STRATEGY.slice(2, 15).toLowerCase()}`;
...
try {
    const res = await api("/v1/users/login-with-wallet", {
        method: "POST",
        auth,
        body: { walletAddress: AGENT_STRATEGY },
    });
